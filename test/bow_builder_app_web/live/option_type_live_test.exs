defmodule BowBuilderAppWeb.OptionTypeLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all option_types", %{conn: conn, option_type: option_type} do
      ...
      assert html =~ "Listing Option types"
    ...
    test "saves new option_type", %{conn: conn} do
      ...
      assert index_live |> element("a", "New Option type") |> render_click() =~
               "New Option type"
      ...
      assert html =~ "Option type created successfully"
    ...
    test "updates option_type in listing", %{conn: conn, option_type: option_type} do
      ...
      assert index_live |> element("#option_types-#{option_type.id} a", "Edit") |> render_click() =~
               "Edit Option type"
      ...
      html = render(index_live)
      assert html =~ "Option type updated successfully"
    ...
    test "deletes option_type in listing", %{conn: conn, option_type: option_type} do
      ...
      assert index_live |> element("#option_types-#{option_type.id} a", "Delete") |> render_click()
  ...
  describe "Show" do
    ...
    test "displays option_type", %{conn: conn, option_type: option_type} do
      ...
      assert html =~ "Show Option type"
      assert html =~ option_type.name
    end

    test "updates option_type within modal", %{conn: conn, option_type: option_type} do
